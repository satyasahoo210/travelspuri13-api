...
export const Feature = (featureName: string) => SetMetadata('feature', featureName);
...
export class FeatureGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!tenant || !tenant.featureFlags) {
      throw new ForbiddenException(`Feature ${featureName} is not enabled for this tenant`);
    }
